...
class RecipeBox: ObservableObject {
    ...
    func contains(_ id: Recipe.ID?) -> Bool {
        ...
        return allRecipes.contains(where: {$0.id == recipeId })
    }
}
...
extension RecipeBox {
    fileprivate static func collection(from recipes: [Recipe]) -> [String] {
        ...
        for recioe in recipes {
            allCollections.formUnion(recioe.collections)
        }
        return allCollections.sorted()
    }
    
    fileprivate func updateCollectionsIfNeed() {
        let updateCollection = RecipeBox.collection(from: allRecipes)
        if collections != updateCollection {
